...
def ret_ret_mcas(pdc):
    def ret_mcas(self, current_iterator_df,sequence_name = None):
        ...
        for idx, _I_ in current_iterator_df.iterrows():
            ...
            #          self.queue.tt.mw_mixing_frequency_R+1]
            freqs = [2398,2340,1,1,2538,2540]
            mcas.start_new_segment(name='init', loop_count = loops)
            mcas.asc(
                A1= '32' in state,
                A2 = '12' in state,
                #length_smpl = 64*int(_I_['init_time']//(loops*64)*12000),
...
def settings(pdc={}):
    ...
    nuclear.parameters = OrderedDict(
        (
            ...
            ('readout',['A2']),
            ('n_pulses', [4]),
            ('tau_2', E.round_length_mus_full_sample(np.linspace(0,10, 2000))),
            ('phase_pi2_2', [0,180])
        )
    )
    nuclear.number_of_simultaneous_measurements = 2*50
